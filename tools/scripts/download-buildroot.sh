...
if [[ ! -e "$CACHED_TARBALL_PATH" ]]; then
    ...
    ALTERNATIVE_URL=http://dl.peridio.com/$TARBALL_NAME
...
EXTRACTED_BASE_NAME=$(basename $EXTRACTED_DIRNAME)
echo $EXTRACTED_BASE_NAME
ln -s "$EXTRACTED_BASE_NAME" "$BUILDROOT_PATH"
